...
With id: one session's vitals — state (exit code once finished), runtime, idle age while active, tokens/cost so far — cost accumulates across the session's turns (for adapters that report usage on the prompt response the per-turn charges are summed; for adapters that stream cumulative usage the largest reported tally wins, so a cold resume never forgets earlier turns), tokens are the latest reported figure (context occupancy for streaming adapters, the turn's total for prompt-response reporters), and a turn that reports no usage at all leaves both untouched, the entry with its base adapter and resolved model, name, session dir and answer path.
...
~/.acpc/                     # root; ACPC_HOME overrides it — the only env var that configures acpc itself
  ...
  sessions/<id>/
    meta.json                # resolved invocation + adapter vias + state, timing, tokens/cost, exit code, stop_reason, prompt snippet, adapter session id (stored command is the entry's base string; --dry-run shows the spawn argv)
