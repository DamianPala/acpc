...
home         ~/.claude (default)
modes        6 · auto (read · delegates) · default (read · delegates) · acceptEdits (execute · delegates)
             plan (read · delegates) · dontAsk (none) · bypassPermissions (all)
...
no id. Existing-terminal operations are not shipped yet, so no decision is stored; inheritance
for those operations arrives with terminal support. Without the creation gate, an adapter
routing edits through the filesystem callback would write to disk at any rung, `none` included.
