...
~/.acpc/                     # root; ACPC_HOME overrides it — the only env var that configures acpc itself
  ...
  sessions/<id>/
    meta.json                # full resolved invocation (everything --dry-run shows, incl. the mode's grants/delegates) + state, timing, tokens/cost, exit code, stop_reason, prompt snippet, adapter session id
...
- **Adapter definitions are TOMLs shipped in the package**, one per adapter — the full contract: `command`, `install_command`, default `home`, `home_env` (the vendor variable the resolved home is exported as, e.g. `CODEX_HOME`), the `[modes]` table, `[presets]`, supported effort levels, `env_passthrough`. A user file in `agents/` with `extends` is a variant; under an adapter's own name it overrides that adapter's fields (e.g. `[presets]`); with a `command` and no `extends` it defines a new adapter. All at the trust level *Agent variants* states.
