...
`--timeout` bounds only how long this client waits, counted from the call and covering acceptance by the daemon, the adapter handshake and the turn itself. It never cancels or changes accepted work. After the deadline the session remains alive under the daemon, or under a detached direct worker when the daemon fallback was used; when acpc observed the turn, it exits 124 with `kind: timeout`, the observed status and a hint to wait, and the result document on stdout carries the transcript content observed so far with `partial: true`. If the turn had not been observed, including a deadline that expires while a cold daemon is still starting the adapter, stdout stays empty and `context.status` reports the observed state, `starting` or `preparing`. `--cancel-after` bounds the work itself. When it expires, ACP cancellation is sent and the observing command reports `operation_failed` with the observed `canceled` status.
...
`wait` observes a background session and prints its answer when the session reaches `succeeded`. A finished session is returned immediately, and its answer is returned on stdout whatever the terminal state was. Any other finished state produces `operation_failed` with the session id and observed status. `--timeout` stops waiting only, leaves the session unchanged and exits 124. When `wait` has observed a turn, it returns the observed status and transcript content on stdout with `partial: true`; a session still in `starting` has no observed turn and produces no result document. If the status cannot be observed, the result is `outcome_unknown` with `status: null` and no result document.
