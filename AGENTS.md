...
- acpc dispatches run `--bg` with `--prompt-file` and `--cwd` of the slice worktree; the orchestrator arms `acpc wait <id> --output-file <file>` in the background in the same turn.
