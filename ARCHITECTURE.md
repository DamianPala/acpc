...
| `schema.py` | The published command surface: the index and one command's detail, both generated by walking the Click tree rather than from a table beside it, plus the global-flag set and the conformance claim. Groups are prefixes unless their no-subcommand callback is explicitly marked with `schema.dispatches_without_command` as dispatching useful work. What Click cannot carry — an argument's description, the `-` that means stdin, a success that is a stream of records — is declared on the command by the `describes` / `reads_stdin` / `emits_record_stream` decorators, the same seam `effects` uses. Imports no `cli`, so the surface can be generated and checked without running a command | *Command surface* |
...
| `render.py` | Condensed event lines, `--prose` view, collection footers on the view stream (`--` prefix, `|`/`·` separators), `--max-output` truncation (UTF-8 boundary, marker line, event-granular for `log`), status line/detail views with idle age | *`log`*, *`status`*, *Output contract* |
