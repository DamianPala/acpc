...
| `client.py` | `AcpcClient` (ACP Client impl): session/update → transcript events, `request_permission` answering via `permissions`, mode-switch handling, answer assembly (message chunks in stream order, thoughts/tools excluded), context-occupancy tracking (`used`/`size`/`peak` from `usage_update`) and per-turn capture of adapter usage; raw response fields stay under transcript-event `meta`, no cost; connection-owned routing captures validated session identity on every raw update, replay generations add per-frame identities, suppression uses validated session/generation tags, and accounted history has a bounded closed-generation backstop | *Output contract* ("the answer"), *Permissions* (runtime), replay silence |
| `usage.py` | Pure per-profile deltas for Claude model usage, Codex context updates and Grok metadata; cumulative model totals, gap and compaction accounting; one quality calculation point | *`status`* (`usage`), *State on disk* |
