...
| `schema.py` | The published command surface: the index and one command's detail, both generated by walking the Click tree rather than from a table beside it, plus the global-flag set and the conformance claim. Groups are prefixes unless their no-subcommand callback is explicitly marked as dispatching useful work. What Click cannot carry — an argument's description, the `-` that means stdin, a success that is a stream of records — is declared on the command by the `describes` / `reads_stdin` / `emits_record_stream` decorators, the same seam `effects` uses. Imports no `cli`, so the surface can be generated and checked without running a command | *Command surface* |
