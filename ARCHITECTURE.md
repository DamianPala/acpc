...
cli.py                          argument parsing, verbs, help, exit codes, TTY detection
  ...
  ├── usage.py                  adapter-specific cumulative usage and compaction accounting
...
| `cli.py` | Verb surface, flag parsing, two-level `--help`, `-V`, TTY vs non-TTY rules, `last` selector, read-only `resolve` previews and session `status`/`list` views, one-time usage-drift note emission at turn summaries, and the one place every failure is reported: it classifies what the layers below raise and renders it through `errors`; `python -m acpc.cli` calls this same `main` | *Command surface*, *`--help`*, *TTY vs non-TTY* |
...
| `sessions.py` | Session ids (4 chars, 32-glyph alphabet, re-roll on collision), permanent identifier reservations, session dirs, `meta.json` lifecycle (atomic replace), cumulative `usage` and nullable drift validation and persistence, locked one-time claim for a session's drift note, state machine + unknown-outcome detection (via `proc`) with a usage gap on liveness transitions, 30s startup grace, per-session lock, nonblocking try-lock context refreshes for the matching active turn, and ephemeral cross-process reservations, `--name` aliases, pre-slot turn rotation and ownership tokens, delivered-prompt markers, locked answer/state finalization including delivery-record incompleteness, delete, prune target resolution and all-target revalidation under stable locks before prune mutation | *State on disk*, *Session states*, *`delete`*, *`prune`* |
...
| `client.py` | `AcpcClient` (ACP Client impl): session/update → transcript events, `request_permission` answering via `permissions`, mode-switch handling, answer assembly (message chunks in stream order, thoughts/tools excluded), context-occupancy tracking (`used`/`size`/`peak` from `usage_update`), pending refresh coalescing at most once per 2 s through the session store, and per-turn capture of adapter usage, size readings and cold-restore state; raw response fields stay under transcript-event `meta`, no cost; connection-owned routing captures validated session identity on every raw update, replay generations add per-frame identities, suppression uses validated session/generation tags, and accounted history has a bounded closed-generation backstop | *Output contract* ("the answer"), *Permissions* (runtime), replay silence |
| `usage.py` | Pure per-profile deltas and per-turn invariants for Claude model usage, Codex context updates and Grok metadata; cumulative model totals, first-drift preservation, gap and compaction accounting; one quality calculation point | *`status`* (`usage`), *State on disk* |
| `runner.py` | One turn end to end: reserved pre-slot cold-resume verification and rotation, route daemon-vs-direct (visible fallback), prompt dispatch, delivery tracking, `--timeout` (wait only, 124), `--cancel-after` (cancel, 1), SIGINT → cancel (130), SIGTERM → detach (143, direct child: cancel) with the handler recording only which signal arrived and its meaning resolved once the route is known, token-checked finalization after a claim, preparation-cancel placeholders, exit codes from stop_reason, `answer.md` + meta finalization for context and cumulative usage, delivery-record incompleteness propagation, auto-prune hook | *`run`*, *`continue`* (turn machinery), *Output contract* (exit codes, client death) |
...
**`run` happy path:** `cli` parses → `registry` resolves the entry + call flags into a full resolution (model, effort, mode, permissions, usage profile, billing, home, env, each with provenance) → `sessions` allocates id + dir, writes `prompt.md` and `meta.json` (status `starting`) → `runner` computes the daemon target (`targets.target_for_call`), asks `daemon_client` for a connection (spawning the daemon if needed), sends the turn → daemon's adapter streams ACP updates → `client` appends transcript events, accumulates the answer and raw turn usage, and refreshes changed context under the session lock while the turn runs → at turn end `usage` derives the cumulative object, then `runner` writes `answer.md` and finalizes meta (status, exit code, stop_reason, context, usage), `output` prints the answer on stdout + summary on stderr. The read-only `resolve` command exposes the same resolution without creating a session.
