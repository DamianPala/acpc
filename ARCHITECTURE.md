...
| `daemon.py` | Daemon process per target: keeps one adapter warm, serves sessions, pre-slot resume preparation, generation-aware callback demultiplexing, `daemon_max_concurrent` prompt slots + queueing, idle TTL expiry, version-skew self-restart, per-target log (adapter stderr), `daemon stop` state transitions | *`daemon`* |
