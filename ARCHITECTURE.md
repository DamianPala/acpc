...
| `daemon_client.py` | Client side of the daemon protocol: ensure-running (spawn+connect race-safe via lock), request/response framing over `ipc`, preparation-only waits, and rules preventing a stale daemon from claiming a session | *`daemon`*, *Session states* |
