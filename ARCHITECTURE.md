...
|--------|------|------------------|
| `cli.py` | Verb surface, flag parsing, two-level `--help`, `-V`, TTY vs non-TTY rules, `last` selector, read-only `resolve` previews, and the one place every failure is reported: it classifies what the layers below raise and renders it through `errors` | *Command surface*, *`--help`*, *TTY vs non-TTY* |
