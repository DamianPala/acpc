...
| `runner.py` | One turn end to end: reserved pre-slot cold-resume verification and rotation, route daemon-vs-direct (visible fallback), prompt dispatch, delivery tracking, `--timeout` (cancel, 124), SIGINT → cancel (130), SIGTERM → detach (143, direct child: cancel), token-checked finalization after a claim, exit codes from stop_reason, `answer.md` + meta finalization, delivery-record incompleteness propagation, auto-prune hook | *`run`*, *`continue`* (turn machinery), *Output contract* (exit codes, client death) |
